...
const STEPS: StringKey[] = ["activationStepOnboarded", "activationStepVerification", "activationStepActive"];

export async function ActivationProgress({ status }: { status: string }) {
  ...
  return (
    <div className="flex flex-col gap-3 border-t border-border px-1 py-3">
      <p className="px-2 text-[10px] font-semibold tracking-wide text-muted-foreground uppercase">{t(lang, "activationTitle")}</p>
        ...
        {STEPS.map((step, index) => {
          ...
          return (
            <div key={step} className="flex items-center gap-2">
              ...
              <span className={`text-xs ${done || current ? "text-foreground" : "text-muted-foreground"}`}>{t(lang, step)}</span>
            </div>
          );
        })}
      </div>
