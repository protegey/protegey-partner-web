...
export function ApiAccessSection({
  ...
  async function handleGenerate() {
    ...
    if (result.error || !result.apiKey) {
      setError(result.error ?? t("commonGenericError"));
      return;
    }
  ...
  return (
    <div className="rounded-md border border-border bg-card p-5">
      ...
      <p className="mb-4 text-sm text-muted-foreground">
        {t("settingsApiAccessDescription")}
      </p>

      <div className="flex flex-col gap-4">
        <div>
          <p className="mb-2 text-xs font-medium text-muted-foreground uppercase">{t("settingsApiKeyLabel")}</p>
          {revealedKey ? (
        ...
        <div className="border-t border-border pt-4">
          <p className="mb-2 text-xs font-medium text-muted-foreground uppercase">{t("settingsWebhookLabel")}</p>
          <p className="mb-2 text-xs text-muted-foreground">
            {t("settingsWebhookDescription")}
          </p>
