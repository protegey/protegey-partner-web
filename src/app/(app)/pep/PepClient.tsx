...
export function PepClient({ designations, initialStatus }: { designations: PepDesignation[]; initialStatus: PepStatus | "all" }) {
  ...
  const field = (name: keyof typeof form, label: string, required = false) => (
    <label className="flex flex-col gap-1 text-xs font-medium text-muted-foreground">
      {label}
      <input required={required} value={form[name]} onChange={(event) => setForm((current) => ({ ...current, [name]: event.target.value }))} className="rounded-md border border-border bg-background px-3 py-2 text-sm font-normal text-foreground outline-none focus:ring-2 focus:ring-ring" />
    </label>
  );
    ...
    <div className="flex flex-col gap-6">
      ...
      <form onSubmit={create} className="rounded-md border border-border bg-card p-5">
        ...
        <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
          {field("externalCustomerId", t("pepCustomerId"), true)}
          ...
          {field("role", t("pepRole"))}{field("jurisdiction", t("pepJurisdiction"))}{field("source", t("pepSource"), true)}
          <label className="flex flex-col gap-1 text-xs font-medium text-muted-foreground sm:col-span-2">{t("pepNotes")}<textarea value={form.notes} onChange={(event) => setForm((current) => ({ ...current, notes: event.target.value }))} className="min-h-20 rounded-md border border-border bg-background px-3 py-2 text-sm font-normal text-foreground outline-none focus:ring-2 focus:ring-ring" /></label>
        </div>
...
function PepRow({ designation, onReview, t }: { designation: PepDesignation; onReview: (designation: PepDesignation, status: PepStatus, notes: string) => Promise<void>; t: (key: never) => string }) {
  ...
  return <tr className="align-top"><td className="px-4 py-3 font-medium text-foreground">{designation.externalCustomerId}</td><td className="px-4 py-3 text-muted-foreground">{t(`pepCategory_${designation.category}` as never)}</td><td className="px-4 py-3 text-muted-foreground">{designation.role ?? "-"}</td><td className="px-4 py-3 text-muted-foreground">{designation.jurisdiction ?? "-"}</td><td className="px-4 py-3 text-muted-foreground">{designation.source}</td><td className="px-4 py-3"><select value={status} onChange={(event) => setStatus(event.target.value as PepStatus)} className="rounded-md border border-border bg-background px-2 py-1 text-xs text-foreground">{statuses.map((value) => <option key={value} value={value}>{t(`pepStatus_${value}` as never)}</option>)}</select></td><td className="min-w-48 px-4 py-3"><textarea value={notes} onChange={(event) => setNotes(event.target.value)} className="min-h-16 w-full rounded-md border border-border bg-background px-2 py-1 text-xs text-foreground" /></td><td className="px-4 py-3"><button type="button" onClick={() => onReview(designation, status, notes)} className="whitespace-nowrap rounded-md border border-border px-3 py-1.5 text-xs font-semibold text-foreground hover:bg-muted">{t("pepReviewButton" as never)}</button></td></tr>;
}
