...
function SubmitButton() {
  ...
  return (
    ...
    >
      {pending ? t("teamSaving") : t("teamSaveChanges")}
    </button>
  );
}
...
export function EditInvitationDialogButton({ invitation, roles }: { invitation: PendingInvitation; roles: AssignableRole[] }) {
  ...
  return (
    <>
      ...
      <Dialog open={open} onClose={() => setOpen(false)} title={t("teamEditDialogTitle")} description={t("teamEditDialogDescription")}>
        <form action={formAction} className="flex flex-col gap-4">
          <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
            ...
            <input
              ...
              defaultValue={invitation.phone ?? undefined}
              required
              className="rounded-md border border-border bg-background px-3 py-2 text-sm text-foreground outline-none focus:ring-2 focus:ring-ring"
            />
          </div>
          ...
          {state.success ? <p className="text-sm text-primary">{t("teamSavedSuccess")}</p> : null}
